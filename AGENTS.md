...
- **Paint cache + hydrate once:** process-scoped `InboxCache` + `LedgerAggregatesStore` paint on tab switch from RAM. `ensureHydrated` / silent delta runs at process warmup (+ reconnect), **not** on every bottom-nav enter. Categorization never full-screen-blocks on `isSyncing`. Manual refresh = `pullInbox` + `refresh`.
